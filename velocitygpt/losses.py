...
class HybridLoss(nn.Module):
    ...
    def forward(self, x, y):
        loss1 = nn.L1Loss()(x, y)
        loss2 = (1 - ssim(x.unsqueeze(1) + 1, y.unsqueeze(1) + 1, data_range=2, size_average=True))
...
class VectorQuantizedVAELoss(nn.Module):
    def __init__(self, config):
        ...
        if config.use_focal_loss:
            self.focal_loss_fn = FocalFrequencyLoss(loss_weight=config.gamma, alpha=config.focal_alpha)
...
class VQVAELoss(nn.Module):
    ...
    def forward(self, x_tilde, images, latent_loss):
        # Reconstruction loss
        loss_recons = self.recon_loss_fn(x_tilde, images)
